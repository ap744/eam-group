...
#print (GC_surface_ammonium.shape)

#Geos-Chem Annual Mean
GC_surface_ammonium = sum(GC_surface_ammonium)/len(GC_surface_ammonium)
...
gc_lon = GC_surface_ammonium['lon']
gc_lat = GC_surface_ammonium['lat']
print (len(gc_lon))
print (len(gc_lat))
print ((gc_lon))
print ((gc_lat))
...
gc_data_ammonium=np.zeros(nsites)
...
for w in range(len(sites_lat)):
	...
	#print (lon_index)
	#print (lat_index)
	gc_data_ammonium[w] = GC_surface_ammonium[lon_index, lat_index]

print (gc_data_ammonium.shape)
print (sites_ammonium_AM.shape)

# quick scatter plot
#plt.plot(sites_ammonium_AM,gc_data_ammonium,'o')
#plt.show()

# Compare DERFA and GEOS-Chem:

#Normalized mean bias
nmb=100.*((sites_ammonium_AM)- np.mean(gc_data_ammonium))/(gc_data_ammonium)
print(' DEFRA NMB= ', nmb)

#correlation
correlate=stats.pearsonr(gc_data_ammonium,sites_ammonium_AM)
...
GC_surface_ammonium.plot(ax=ax,cmap=cmap,vmin = 0,vmax =3,
								cbar_kwargs={'shrink': 0.5, 
											'pad' : 0.01,
											'label': 'GEOS-Chem ammonium ($\mu$g m$^{-3}$)',
...
#ax.annotate('ammonium', xy=(10.1, 42.5), xytext=(10, 42.5),fontsize = 30)

ax.set_title('DEFRA and GEOS-Chem Particulate ammonium')



colorbar.ax.tick_params(labelsize=10) 
colorbar.ax.xaxis.label.set_size(10)
plt.savefig('/home/a/ap744/scratch_alok/python_work/'+Today_date+'ammonium_GEOS-Chem_DEFRAspatial.png',bbox_inches='tight')


#scatter plot 

#ax = plt.subplot(1,2,2);
#ax.scatter(x=sites_ammonium_AM, y=gc_data_ammonium, c=gc_data_ammonium, 
...
title_list2 = 'DERFA & GEOS-Chem ammonium'
...
plt.xlabel("GEOS-Chem ammonium ($\mu$g m$^{-3}$)", fontsize = 30)
plt.ylabel("DEFRA_ammonium ($\mu$g m$^{-3}$)", fontsize = 30)
