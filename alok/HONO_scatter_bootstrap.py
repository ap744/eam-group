# This code compare GEOS-Chem model and DEFRA sites HONO 
...
#read UKEAP HONO datasets here scratch_alok -> /scratch/uptrop/ap744
path='/scratch/uptrop/ap744/UKEAP_data/UKEAP_AcidGases_Aerosol/UKEAP_HONO/HONO_with_somedata/'
HONO_files=glob.glob(path + '28-UKA0*-2016_*.csv')
print (HONO_files)
...
for f in HONO_files:
	...
	sitesA["HONO_annual_mean"] = mean_A
	sitesA["HONO_mam_mean"] = mean_mam
	sitesA["HONO_jja_mean"] = mean_jja
	sitesA["HONO_son_mean"] = mean_son
	sitesA["HONO_djf_mean"] = mean_djf
	#print (sitesA.head(50))
	
	x.append(
	{
		'UK-AIR_ID':f[90:98],
		'HONO_annual_mean':mean_A,
		'HONO_mam_mean':mean_mam,
		'HONO_jja_mean':mean_jja,
		'HONO_son_mean':mean_son,
		'HONO_djf_mean':mean_djf
		}
		)
	#print (x)
...
#df_merge_col.to_csv(r'/home/a/ap744/scratch_alok/python_work/HONO_annual_mean.csv')
...
sites_HONO_AM = df_merge_colA['HONO_annual_mean']

#seasonal mean data
sites_HONO_mam = df_merge_colA['HONO_mam_mean']
sites_HONO_jja = df_merge_colA['HONO_jja_mean']
sites_HONO_son = df_merge_colA['HONO_son_mean']
sites_HONO_djf = df_merge_colA['HONO_djf_mean']
...
#HONO sufrace layer
GC_surface_HONO = [data['SpeciesConc_HNO2'].isel(time=0,lev=0) for data in Species]
print (GC_surface_HONO)
...
MW_AIR = 28.9644e-3
# convert unit for HONO (dry mol/mol to ug/m3)
...
surface_HONO_mass  = [x*y*47/(6.022*1e11) for (x,y) in zip(GC_surface_HONO,surface_AIRNUMDEN)]
print (surface_HONO_mass)

#Geos-Chem Annual Mean
GC_surface_HONO_AM = sum(surface_HONO_mass)/len(surface_HONO_mass)
#print (GC_surface_HONO_AM,'AnnualMean')
print (GC_surface_HONO_AM.shape,'AnnualMean shape')

#Geos-Chem seasonal Mean
GC_surface_HONO_mam = sum(surface_HONO_mass[2:5])/len(surface_HONO_mass[2:5])
#print (GC_surface_HONO_mam.shape, 'MAM-shape')

GC_surface_HONO_jja = sum(surface_HONO_mass[5:8])/len(surface_HONO_mass[5:8])
#print (GC_surface_HONO_jja)

GC_surface_HONO_son = sum(surface_HONO_mass[8:11])/len(surface_HONO_mass[8:11])
#print (GC_surface_HONO_son)

GC_surface_HONO_jf = sum(surface_HONO_mass[0:2])/len(surface_HONO_mass[0:2])
print (GC_surface_HONO_jf, 'jf_shape')

GC_surface_HONO_d = surface_HONO_mass[11]
print (GC_surface_HONO_d, 'd_shape')

#mean of JF and Dec using np.array --> creating problem in plotting
#GC_surface_HONO_djf_a = np.array([GC_surface_HONO_jf,GC_surface_HONO_d])
#GC_surface_HONO_djf = np.nanmean(GC_surface_HONO_djf_a,axis=0)
#print (GC_surface_HONO_djf, 'djf_shape')


GC_surface_HONO_djf = (GC_surface_HONO_d+GC_surface_HONO_jf)/2
print (GC_surface_HONO_djf, 'djf_shape')
...
gc_lon = GC_surface_HONO_AM['lon']
gc_lat = GC_surface_HONO_AM['lat']
print (len(gc_lon))
print (len(gc_lat))
print ((gc_lon))
print ((gc_lat))
...
gc_data_HONO_annual=np.zeros(nsites)

gc_data_HONO_mam=np.zeros(nsites)
gc_data_HONO_jja=np.zeros(nsites)
gc_data_HONO_son=np.zeros(nsites)
gc_data_HONO_djf=np.zeros(nsites)
...
for w in range(len(sites_lat)):
	...
	#print (lon_index)
	#print (lat_index)
	gc_data_HONO_annual[w] = GC_surface_HONO_AM[lon_index, lat_index]
	gc_data_HONO_mam[w] = GC_surface_HONO_mam[lon_index, lat_index]
	gc_data_HONO_jja[w] = GC_surface_HONO_jja[lon_index, lat_index]
	gc_data_HONO_son[w] = GC_surface_HONO_son[lon_index, lat_index]
	gc_data_HONO_djf[w] = GC_surface_HONO_djf[lon_index, lat_index]

print (gc_data_HONO_annual.shape)
print (sites_HONO_AM.shape)

# quick scatter plot
#plt.plot(sites_HONO_AM,gc_data_HONO_annual,'o')
#plt.show()

# Compare DERFA and GEOS-Chem:

#Normalized mean bias
#nmb_Annual=100.*((np.nanmean(sites_HONO_AM))- np.nanmean(gc_data_HONO_annual))/np.nanmean(gc_data_HONO_annual)
#nmb_mam=100.*((np.nanmean(sites_HONO_mam))- np.nanmean(gc_data_HONO_mam))/np.nanmean(gc_data_HONO_mam)
#nmb_jja=100.*((np.nanmean(sites_HONO_jja))- np.nanmean(gc_data_HONO_jja))/np.nanmean(gc_data_HONO_jja)
#nmb_son=100.*((np.nanmean(sites_HONO_son))- np.nanmean(gc_data_HONO_son))/np.nanmean(gc_data_HONO_son)
#nmb_djf=100.*((np.nanmean(sites_HONO_djf))- np.nanmean(gc_data_HONO_djf))/np.nanmean(gc_data_HONO_djf)

nmb_Annual=100.*((np.nanmean(gc_data_HONO_annual))- np.nanmean(sites_HONO_AM))/np.nanmean(sites_HONO_AM)
nmb_mam=100.*((np.nanmean(gc_data_HONO_mam))- np.nanmean(sites_HONO_mam))/np.nanmean(sites_HONO_mam)
nmb_jja=100.*((np.nanmean(gc_data_HONO_jja))- np.nanmean(sites_HONO_jja))/np.nanmean(sites_HONO_jja)
nmb_son=100.*((np.nanmean(gc_data_HONO_son))- np.nanmean(sites_HONO_son))/np.nanmean(sites_HONO_son)
nmb_djf=100.*((np.nanmean(gc_data_HONO_djf))- np.nanmean(sites_HONO_djf))/np.nanmean(sites_HONO_djf)
...
#correlation
correlate_annual=stats.pearsonr(gc_data_HONO_annual,sites_HONO_AM)

# dropping nan values and compute correlation
nas_mam = np.logical_or(np.isnan(gc_data_HONO_mam), np.isnan(sites_HONO_mam))
correlate_mam = stats.pearsonr(gc_data_HONO_mam[~nas_mam],sites_HONO_mam[~nas_mam])

nas_jja = np.logical_or(np.isnan(gc_data_HONO_jja), np.isnan(sites_HONO_jja))
correlate_jja = stats.pearsonr(gc_data_HONO_jja[~nas_jja],sites_HONO_jja[~nas_jja])

nas_son = np.logical_or(np.isnan(gc_data_HONO_son), np.isnan(sites_HONO_son))
correlate_son = stats.pearsonr(gc_data_HONO_son[~nas_son],sites_HONO_son[~nas_son])

nas_djf = np.logical_or(np.isnan(gc_data_HONO_djf), np.isnan(sites_HONO_djf))
correlate_djf = stats.pearsonr(gc_data_HONO_djf[~nas_djf],sites_HONO_djf[~nas_djf])
print('Correlation = ',correlate_annual)


x11 = gc_data_HONO_annual
y11 = sites_HONO_AM
print (x11)
print (y11)
...
#ind=np.where((gc_data_HONO_annual!=0)&(sites_HONO_AM!=0))
#print (ind)
...
#scatter plot 
title_list = 'DEFRA and GEOS-Chem HONO'
title_list1 = 'DEFRA & GEOS-Chem HONO Annual'
title_list2 = 'DEFRA & GEOS-Chem HONO MAM'
title_list3 = 'DEFRA & GEOS-Chem HONO JJA'
title_list4 = 'DEFRA & GEOS-Chem HONO SON'
title_list5 = 'DEFRA & GEOS-Chem HONO DJF'
...
plt.xlabel("GEOS-Chem HONO ($\mu$g m$^{-3}$)", fontsize = 20)
plt.ylabel("DEFRA_HONO ($\mu$g m$^{-3}$)", fontsize = 20)
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'HONO_GEOS-Chem_DEFRAscatter_annual_bootstrap.png',bbox_inches='tight')
#plt.show()
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'HONO_GEOS-Chem_DEFRAscatter_seasonal_bootstrap.png',bbox_inches='tight')
plt.show()
