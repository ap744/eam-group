...
Avegadro_No = 6.022*1e23
# convert unit for NH3 (dry mol/mol to ug/m3)
surface_AIRDEN = [data['Met_AIRDEN'].isel(time=0,lev=0) for data in StateMet]
surface_AIRNUMDEN = (surface_AIRDEN/28.96)*Avegadro_No
surface_ammonia_mass  = [x*y*17/(6.022*1e23) for (x,y) in zip(GC_surface_ammonia,surface_AIRNUMDEN)]
...
GC_surface_ammonia_d = surface_ammonia_mass[0]
...
nmb_Annual=100.*((sites_ammonia_AM)- np.mean(gc_data_ammonia_annual))/(gc_data_ammonia_annual)
nmb_mam=100.*((sites_ammonia_mam)- np.mean(gc_data_ammonia_mam))/(gc_data_ammonia_mam)
nmb_jja=100.*((sites_ammonia_jja)- np.mean(gc_data_ammonia_jja))/(gc_data_ammonia_jja)
nmb_son=100.*((sites_ammonia_son)- np.mean(gc_data_ammonia_son))/(gc_data_ammonia_son)
nmb_djf=100.*((sites_ammonia_djf)- np.mean(gc_data_ammonia_djf))/(gc_data_ammonia_djf)
...
ax.set_title('DEFRA and GEOS-Chem Particulate ammonia (Annual)')
...
ax.set_title('DEFRA and GEOS-Chem Particulate ammonia (mam)')
PCM=ax.get_children()[2] #get the mappable, the 1st and the 2nd are the x and y axes
...
ax.set_title('DEFRA and GEOS-Chem Particulate ammonia (jja)')
PCM=ax.get_children()[2] #get the mappable, the 1st and the 2nd are the x and y axes
...
ax.set_title('DEFRA and GEOS-Chem Particulate ammonia (son)')
PCM=ax.get_children()[2] #get the mappable, the 1st and the 2nd are the x and y axes
...
ax.set_title('DEFRA and GEOS-Chem Particulate ammonia (djf)')
PCM=ax.get_children()[2] #get the mappable, the 1st and the 2nd are the x and y axes
