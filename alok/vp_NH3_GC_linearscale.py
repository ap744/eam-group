...
def mask_weight(region_key,lon,lat,return_option,reverse=False):
	...
	box_region_dic={'Wales':[4.3,3,51.5,53],'South_England':[2.5,1,51,52],'East_England':[0,1.5,52,53],'NE_England':[2.5,1.5,54.5,55.5],'All':[0,360,-90,90],'ASIA':[65,145,5,45],'US':[240,290,30,50],'ARCTIC':[0,360,60,90],'TROPICS':[0,360,-28,28],'EUROPE':[0,40,30,70],}
...
def East_England_ts_mon():
	...
	print (East_England_NH3_GC.shape)
...
levels = np.linspace(0,3.5,11)
#levels = np.logspace(-2.5, 0.6, 11)
print (levels)
tick_levels = [0, 0.35, 0.7, 1.05, 1.4, 1.75, 2.1, 2.45, 2.8, 3.15, 3.5]
#tick_levels =[0.003, 0.006, 0.013, 0.027, 0.055, 0.112, 0.229,0.467,0.954,1.949,3.981]

fig = plt.figure(facecolor='White',figsize=[1,11]);pad= 0.5; 
...
plt.contourf(wales_NH3_GC_vp, levels,  cmap = cm.jet,extend='both')
...
plt.contourf(NE_England_NH3_GC_vp, levels, cmap = cm.jet,extend='both')
#plt.contourf(wales_NH3_GC_vp, levels, cmap = cm.jet)
#plt.contourf(wales_NH3_GC_vp, cmap = cm.jet)
...
plt.contourf(South_England_NH3_GC_vp, levels, cmap = cm.jet,extend='both')
...
plt.contourf(East_England_NH3_GC_vp, levels, cmap = cm.jet,extend='both')
...
plt.subplots_adjust(left=0.05, bottom=0.07, right=0.98, top=0.97, wspace=0.21, hspace=0.12);
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'vertical_profile_linearscale.png',bbox_inches='tight')   ##########	
plt.show()
