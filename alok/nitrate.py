...
#print (GC_surface_nitrate.shape)

#Geos-Chem Annual Mean
GC_surface_nitrate = sum(GC_surface_nitrate)/len(GC_surface_nitrate)
...
gc_lon = GC_surface_nitrate['lon']
gc_lat = GC_surface_nitrate['lat']
print (len(gc_lon))
print (len(gc_lat))
print ((gc_lon))
print ((gc_lat))
...
gc_data_nitrate=np.zeros(nsites)
...
for w in range(len(sites_lat)):
	...
	#print (lon_index)
	#print (lat_index)
	gc_data_nitrate[w] = GC_surface_nitrate[lon_index, lat_index]

print (gc_data_nitrate.shape)
print (sites_nitrate_AM.shape)

# quick scatter plot
#plt.plot(sites_nitrate_AM,gc_data_nitrate,'o')
#plt.show()

# Compare DERFA and GEOS-Chem:

#Normalized mean bias
nmb=100.*((sites_nitrate_AM)- np.mean(gc_data_nitrate))/(gc_data_nitrate)
print(' DEFRA NMB= ', nmb)

#correlation
correlate=stats.pearsonr(gc_data_nitrate,sites_nitrate_AM)
...
GC_surface_nitrate.plot(ax=ax,cmap=cmap,vmin = 0,vmax =3,
...
#ax.annotate('nitrate', xy=(10.1, 42.5), xytext=(10, 42.5),fontsize = 30)

ax.set_title('DEFRA and GEOS-Chem Particulate nitrate')



colorbar.ax.tick_params(labelsize=10) 
colorbar.ax.xaxis.label.set_size(10)
plt.savefig('/home/a/ap744/scratch_alok/python_work/'+Today_date+'nitrate_GEOS-Chem_DEFRAspatial.png',bbox_inches='tight')


#scatter plot 

#ax = plt.subplot(1,2,2);
#ax.scatter(x=sites_nitrate_AM, y=gc_data_nitrate, c=gc_data_nitrate, 
...
title_list2 = 'DERFA & GEOS-Chem nitrate'
...
plt.xlabel("GEOS-Chem nitrate ($\mu$g m$^{-3}$)", fontsize = 30)
plt.ylabel("DEFRA_nitrate ($\mu$g m$^{-3}$)", fontsize = 30)
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'nitrate_GEOS-Chem_DEFRAscatter.png',bbox_inches='tight')
plt.show()
