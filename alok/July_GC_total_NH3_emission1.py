...
sum_regridded_emission  = np.nansum(np.nansum(np.nansum(regridded_data_emission.data,axis=2),axis=1),axis=0)
...
sum_regridded_emission2  = np.nansum(np.nansum(np.nansum(regridded_data_emission2.data,axis=2),axis=1),axis=0)
...
july_total_NH3_emission = regridded_data_emission_uk_GC[6,:,:].copy()
...
plt.savefig('/scratch/uptrop/ap744/python_work/'+Today_date+'July_GC_Total_NH3_emission1.png',bbox_inches='tight')
plt.show()
